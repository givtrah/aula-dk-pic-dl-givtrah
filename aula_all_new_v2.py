...
class Arguments:
    def __init__(self):
        parser = argparse.ArgumentParser(description='Download images from aula.dk - use MitID to login or it might fail! Hard limit of looking at first 1000 albums!')
        parser.add_argument(
            '-o',
            required=True,
            default='output',
...
# 2023-10-21: Changed because I don't care about the time, only the date (in order to rename the file...)
    image_creation_fulltime = datetime.strptime(file_data['created'], '%Y-%m-%dT%H:%M:%S%z')
...
# added 2023-10-21 - add date + album_name to filename
            better_image_name = image_data["image_creation_time"] + "_" + album.name + "_" + file_data['name']
...
# below was changed in order to give correct name to the file
            image_path = os.path.join(image_data["image_directory_path"], better_image_name)
...
# Rename file if file exists - happens VERY rarely with certain video files named the same (and from the same date... - this is a qucik&dirty fix and can be improved)
            if os.path.exists(image_path):
                image_new_name = image_data["image_creation_time"] + "_" + album.name + "_" + str(random.randint(1,9999999)) + "_" + file_data['name']
                image_path = os.path.join(image_data["image_directory_path"], image_new_name)
