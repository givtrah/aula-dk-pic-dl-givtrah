...
# addition imports 2023-10-21 givtrah
import re
import random
...
class Arguments:
    def __init__(self):
        parser = argparse.ArgumentParser(
            description="Download images from aula.dk - use MitID to login or it might fail! Hard limit of looking at first 1000 albums!"
        )
        parser.add_argument(
            "-o", required=True, default="output", help="Download images in this folder"
        )
        parser.add_argument(
            "-d",
            required=True,
        ...
        self.cutoff_date = datetime.fromisoformat(args.d).date()
    ...
    def print_arguments(self):
        ...
        console.print(
            f"  cutoffDate: {self.cutoff_date.strftime('%Y-%m-%d')}", style=param_style
        )
        console.print()
...
def add_exif_creation_time(image_path, creation_time):
    ...
    if is_jpeg:
        ...
        exif_ifd = {
            piexif.ExifIFD.DateTimeOriginal: creation_time.strftime(
                "%Y:%m:%d %H:%M:%S"
            ),
        }
...
def get_albums_from_gallery(institution_profile_ids, cutoff_date):
    ...
    for album in albums_with_id:
        ...
        if not is_album_created_after_cutoff_date:
            continue
        pictures = client.get_pictures(
            institution_profile_ids, album["id"], additional_params
        )
        if pictures:
...
def get_albums_from_messages(cutoff_date):
    ...
    for thread in threads:
        ...
        thread_latest_message_time = parse_datetime(
            thread["latestMessage"]["sendDateTime"]
        )
        is_thread_updated_after_cutoff_date = (
            thread_latest_message_time.date() >= cutoff_date
        )
...
def get_threads(cutoff_date):
    ...
    while threads_response["moreMessagesExist"]:
        ...
        threads_response = client.get_threads(threads_page_param)
        threads += threads_response["threads"]
        last_thread_update_time = parse_datetime(
            threads[-1]["latestMessage"]["sendDateTime"]
        )
...
def get_image_data(album: AlbumToDownload, output_directory, file_data):
    ...
    album_directory_name = album.creation_date.strftime("%Y-%m-%d") + "_" + album.name
    ...
    # 2023-10-21: Changed because I don't care about the time, only the date (in order to rename the file...)
    image_creation_fulltime = datetime.strptime(
        file_data["created"], "%Y-%m-%dT%H:%M:%S%z"
    )
    image_creation_time = image_creation_fulltime.strftime("%Y-%m-%d")
    image_response = requests.get(file_data["url"], timeout=30)

    # 2023-10-21: Below added in to replace the stupid recursive directory shit
    image_directory_path = album_directory_path
    # 2023-10-21: Screw the below shit, I don't want recursive directories! I want filenames to match instead!
...
def main():
    ...
    for album in track(albums_to_download, "Albums to download..."):
        ...
        for picture in album.pictures:
            ...
            # added 2023-10-21 - add date + album_name to filename
            better_image_name = (
                image_data["image_creation_time"]
                + "_"
                + album.name
                + "_"
                + file_data["name"]
            ...
            # below was changed in order to give correct name to the file
            image_path = os.path.join(
                image_data["image_directory_path"], better_image_name
            )
            ...
            if os.path.exists(image_path):
                ...
                image_path = os.path.join(
                    image_data["image_directory_path"], image_new_name
                )
