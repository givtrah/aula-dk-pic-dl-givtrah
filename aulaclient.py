...
class AulaClient:
    baseUrl = "https://www.aula.dk/api/v21/"
    defaultLimit = 10

    def __init__(self, apiVersion=21, cookieString=None):
        if not cookieString:
            ...
            cookie = requests.cookies.create_cookie(
                domain="aula.dk", name="aula", value=cookieString
            )
    ...
    def get_threads(self, custom_params=None):
        if custom_params is None:
            custom_params = {}
        default_params = {"sortOn": "date", "orderDirection": "desc", "page": 0}
        params = self.__merge_params(
            default_params, custom_params, "messaging.getThreads"
        )
    ...
    def get_messages_for_thread(self, thread_id, custom_params=None):
        if custom_params is None:
            custom_params = {}
        default_params = {"threadId": thread_id, "page": 0}
        params = self.__merge_params(
            default_params, custom_params, "messaging.getMessagesForThread"
        )
